...
export default function TemplateCertificado ({ certificadoId, evento, professor, data, showDelete=true }) {
    ...
    const handleDelete = async () => {
        try {
            await axios.delete(`https://pdghaebgjd.us-east-1.awsapprunner.com/api/presence/${certificadoId}`, {
                headers: {
    ...
    const handleDownload = async () => {
        try {
            const response = await axios.get(`https://pdghaebgjd.us-east-1.awsapprunner.com/api/create-certificate/${certificadoId}`, {
                headers: {
                    Authorization: `Bearer ${localStorage.getItem("token")}`
                }
            });
