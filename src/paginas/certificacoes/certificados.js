...
export default function Certificacao() {
    ...
    useEffect(() => {
        axios.get(`https://pdghaebgjd.us-east-1.awsapprunner.com/api/events/${eventId}`)
        .then(response => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get('https://pdghaebgjd.us-east-1.awsapprunner.com/api/users-list');
