...
export default function Certificacao() {
    ...
    useEffect(() => {
        axios.get(`https://6ymi3mpnkb.us-east-1.awsapprunner.com/api/events/${eventId}`)
        .then(response => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
            try {
                const response = await axios.get('https://6ymi3mpnkb.us-east-1.awsapprunner.com/api/users-list');
